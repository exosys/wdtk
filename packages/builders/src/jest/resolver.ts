...
const { runCLI } = require('jest-resolve/build/default_resolver');
...
function getCompilerSetup(rootDir: string) {
  ...
  const config = ts.parseJsonConfigFileContent(readResult.config, ts.sys, dirname(tsConfigPath));
...
module.exports = function(path: string, options: ResolveOptions) {
  ...
  try {
    ...
    return ts.resolveModuleName(path, options.basedir, compilerOptions, host).resolvedModule.resolvedFileName;
  }
};
