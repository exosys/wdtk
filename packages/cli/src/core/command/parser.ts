...
import { Arguments, Option, OptionType, Value } from './defs';

export class ParseArgumentException extends BaseException {
  constructor(public readonly comments: string[], public readonly parsed: Arguments, public readonly ignored: string[]) {
...
function _coerceType(str: string | undefined, type: OptionType, v?: Value): Value | undefined {
  switch (type) {
    ...
    case OptionType.Array:
      return Array.isArray(v) ? v.concat(str || '') : v === undefined ? [str || ''] : [v + '', str || ''];
    ...
    if (v !== undefined) {
      if (parsedOptions[option.name] !== v) {
        ...
        if (option.deprecated !== undefined && option.deprecated !== false) {
          warnings.push(
            `Option ${JSON.stringify(option.name)} is deprecated${typeof option.deprecated == 'string' ? ': ' + option.deprecated : ''}.`
          );
        }
      }
    } else {
 ...
 */
export function parseArguments(args: string[], options: Option[] | null, logger?: logging.Logger): Arguments {
  if (options === null) {
    options = [];
  }
