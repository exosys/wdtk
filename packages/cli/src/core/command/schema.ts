...
import { CommandConstructor, CommandDescription, CommandScope, Option, OptionType, SubCommandDescription, Value } from './defs';
...
function _getEnumFromValue<E, T extends E[keyof E]>(value: json.JsonValue, enumeration: E, defaultValue: T): T {
...
export async function parseJsonSchemaToOptions(registry: json.schema.SchemaRegistry, schema: json.JsonObject): Promise<Option[]> {
  ...
  function visitor(
    ...
    const required = json.isJsonArray(current.required) ? current.required.indexOf(name) != -1 : false;
    const aliases = json.isJsonArray(current.aliases) ? [...current.aliases].map(x => '' + x) : current.alias ? ['' + current.alias] : [];
