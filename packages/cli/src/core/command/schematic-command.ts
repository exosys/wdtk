 ...
 */
import { experimental, json, logging, normalize, schema, strings, tags, terminal, virtualFs } from '@angular-devkit/core';
import { NodeJsSyncHost } from '@angular-devkit/core/node';
import { DryRunEvent, Engine, SchematicEngine, UnsuccessfulWorkflowExecution, workflow } from '@angular-devkit/schematics';
import {
...
import { getProjectByCwd, getSchematicDefaults, getWorkspace, getWorkspaceRaw } from '../../util/config';
import { parseJsonSchemaToOptions } from './schema';
import { getPackageManager } from '../../util/package-manager';
import { BaseCommandOptions, AbstractCommand } from './abstract-command';
import { Arguments, CommandContext, CommandDescription, Option } from './defs';
...
export abstract class SchematicCommand<T extends BaseSchematicSchema & BaseCommandOptions> extends AbstractCommand<T> {
  ...
  // protected collectionName = './lib/schematics/collection.json';
  // protected collectionName = systemPath.resolve(__dirname, '..', 'schematics/collection.json');
  protected collectionName = '@wdtk/schematics';
  ...
  public async initialize(options: T & Arguments) {
    ...
    if (this.schematicName) {
      ...
      const options = await parseJsonSchemaToOptions(this._workflow.registry, schematic.description.schemaJson || {});
  ...
  async printHelpUsage() {
    ...
    if (schematicNames.length == 1) {
      ...
      //FIXME use product.shortName instead of wx
      this.logger.info(tags.oneLine`
        usage: wx ${this.description.name} ${displayName}${argDisplay}
  ...
  protected createWorkflow(options: BaseSchematicSchema): workflow.BaseWorkflow {
    ...
    workflow.registry.addSmartDefaultProvider('projectName', () => {
      if (this._workspace) {
        try {
          return this._workspace.getProjectByPath(normalize(process.cwd())) || this._workspace.getDefaultProjectName();
        } catch (e) {
    ...
    if (collectionName !== this.collectionName) {
      ...
      if (ast) {
        ...
        if (positions.length > 0) {
          ...
          const locations = positions.map((p, i) => `${i + 1}) Line: ${p.line + 1}; Column: ${p.character + 1}`).join('\n');
