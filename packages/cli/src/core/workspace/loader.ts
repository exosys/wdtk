...
import { Path, basename, dirname, experimental, normalize, virtualFs } from '@angular-devkit/core';
import { findUp } from './util';

export class WorkspaceLoader {
  // TODO: add remaining fallbacks.
  private _configFileNames = [normalize('.angular.json'), normalize('angular.json')];
  ...
  private _getProjectWorkspaceFilePath(projectPath?: string): Path {
    ...
    const workspaceFilePath =
      (projectPath && findUp(this._configFileNames, projectPath)) ||
