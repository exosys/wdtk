...
import { findUp } from '../core/workspace';
...
function projectFilePath(projectPath?: string): string | null {
  ...
  return (projectPath && findUp(configNames, projectPath)) || findUp(configNames, process.cwd()) || findUp(configNames, __dirname);
}
...
export function getWorkspace(level: 'local' | 'global' = 'local'): experimental.workspace.Workspace | null {
...
export function getWorkspaceRaw(level: 'local' | 'global' = 'local'): [JsonAstObject | null, string | null] {
  ...
  let content = '';
  new NodeJsSyncHost().read(normalize(configPath)).subscribe(data => (content = virtualFs.fileBufferToString(data)));
...
export function validateWorkspace(json: JsonObject) {
  const workspace = new experimental.workspace.Workspace(normalize('.'), new NodeJsSyncHost());
...
export function migrateLegacyGlobalConfig(): boolean {
  ...
  if (homeDir) {
    ...
    if (existsSync(legacyGlobalConfigPath)) {
      ...
      if (
        legacy.defaults &&
        typeof legacy.defaults == 'object' &&
        !Array.isArray(legacy.defaults) &&
        legacy.defaults.schematics &&
        typeof legacy.defaults.schematics == 'object' &&
      ...
      if (legacy.warnings && typeof legacy.warnings == 'object' && !Array.isArray(legacy.warnings)) {
...
export function getSchematicDefaults(collection: string, schematic: string, project?: string | null): {} {
  let result = {};
